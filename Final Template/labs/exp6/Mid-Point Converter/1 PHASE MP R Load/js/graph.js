  ...
  if (
    ...
    Plotly.newPlot(
      ...
      ],
      {
        ...
        yaxis: {
          range: [
            -1 * ((parseInt(values["AC1"]["volt"]))*2.2+1+0.1*parseInt(values["AC1"]["volt"])),
            ((parseInt(values["AC1"]["volt"]))*2.2+1+0.1*parseInt(values["AC1"]["volt"])),
          ],
          title: "<b>Voltage(V)</b>",
          fixedrange: true,
        },
    ...
    if (sin_fun > 0) {
      ...
      if (x <= ending1) {
        if (x >= starting1) {
          ...
          load_voltage.push(0);
          current.push(0);
          thyristor_voltage.push(sin_fun / 1.1);
          thyristor_voltage2.push(-1 * (sin_fun / 1.1));
        }
      } else if (flag) {
        load_voltage.push(-1 * sin_fun);
        thyristor_voltage.push(sin_fun * 2);
